...
func GetGrid(Email string) (models.Grid, error) {
	collection := client.Database("zine").Collection("grids")

	// Get the user ID from the email
	userCollection := client.Database("zine").Collection("users")
	var user models.User
	userFilter := bson.M{"email": Email}
	err := userCollection.FindOne(context.TODO(), userFilter).Decode(&user)
	if err != nil {
		return models.Grid{}, errors.New("user not found")
	}
	// Use the user ID to filter the grid
	gridFilter := bson.M{"user_id": user.ID}
	// Retrieve the grid
	var grid models.Grid
	err = collection.FindOne(context.TODO(), gridFilter).Decode(&grid)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return models.Grid{}, errors.New("grid not found")
		}
		return models.Grid{}, err
	}
	return grid, err
}

func UpdateGrid(Email string, Date string) error {
	collection := client.Database("zine").Collection("grids")
	...
	// Append the date to the grid's dates array
	if grid.Dates == nil {
		grid.Dates = make([]string, 0)
	}
	grid.Dates = append(grid.Dates, Date)

	// Update the grid in the database
	update := bson.M{"$set": bson.M{"dates": grid.Dates}}
	_, err = collection.UpdateOne(context.TODO(), gridFilter, update)

	return err
}

func CreateGrid(userID string) error {
	collection := client.Database("zine").Collection("grids")

	_, err := collection.InsertOne(context.TODO(), bson.M{"user_id": userID})
	return err
}
